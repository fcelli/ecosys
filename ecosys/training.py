...
class Trainer:
    ...
    def run_episode(
        ...
        # Initialise tensors containing the action probabilities, the critic values and the rewards
    ...
    # Wrap Gym's `env.step` call as an operation in a TensorFlow function. This allows it to be included in a callable TensorFlow graph.
    def env_step(self, action: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        '''Returns state, reward and done flag given an action.'''
        state, reward, done = self.env.step(action)
        return (state.astype(np.int8), np.array(reward, np.float32), np.array(done, np.int8))
